...
riot$1.tag2('router', '<yield></yield>', '', '', function(opts) {
    var this$1 = this;
...
riot$1.tag2('route', '<virtual if="{show}"><yield></yield></virtual>', '', '', function(opts) {
    var this$1 = this;
    ...
    this.parent.route(opts.path, function () {
      ...
      this$1.one('updated', function () {
        flatten(this$1.tags).forEach(function (tag$$1) {
          tag$$1.trigger.apply(tag$$1, [ 'route' ].concat( args ));
          tag$$1.update();
        });
      });
    ...
    function flatten(tags) {
      return Object.keys(tags)
        ...
        .reduce(function (acc, tag$$1) { return acc.concat(tag$$1); }, [])
    }
});

riot$1.tag2('app', '<h1>shinkan.today</h1> <router> <route path="*/*"><app-detail></app-detail></route> <route path="*"><app-search></app-search></route> <route path="/"><app-search></app-search></route> </router>', 'app,[data-is="app"]{ display: block; padding-bottom: 50px; } app h1,[data-is="app"] h1{ position: fixed; bottom: 0; left: 0; right: 0; background: #2c3644; font-size: 90%; font-weight: normal; text-align: center; padding: 8px; margin: 0; color: rgba(255, 255, 255, .5); border-top: 1px solid rgba(255, 255, 255, .1); }', '', function(opts) {
});
...
riot$1.tag2('app-search', '<header> <h2>{jDate}<span>全{books.length}件</span></h2> <a class="prev" if="{prev}" href="#{prev}">前日</a> <span class="prev" if="{!prev}">前日</span> <a class="next" if="{next}" href="#{next}">翌日</a> <span class="next" if="{!next}">翌日</span> </header> <ul> <li each="{visibleBooks}" riot-style="background-image: url({cover})"> <a href="#{date}/{isbn}" title="{title}">{title}</a> </li> </ul> <button if="{visibleBooks.length < books.length}" onclick="{more}">もっと表示</button>', 'app-search,[data-is="app-search"]{ display: block; } app-search header,[data-is="app-search"] header{ text-align: center; margin: 0 0 5px; border-bottom: 1px solid rgba(255, 255, 255, .1); color: rgba(255, 255, 255, .8); position: relative; } app-search header > h2,[data-is="app-search"] header > h2{ font-size: 100%; padding: 15px 10px; } app-search header > h2 > span,[data-is="app-search"] header > h2 > span{ font-size: 90%; font-weight: normal; margin-left: 1em; } app-search header > .prev,[data-is="app-search"] header > .prev,app-search header > .next,[data-is="app-search"] header > .next{ position: absolute; background: rgba(255, 255, 255, .1); border: 1px solid rgba(255, 255, 255, .3); border-radius: 3px; color: rgba(255, 255, 255, .8); text-align: center; padding: 5px 10px; top: 10px; } app-search header > .prev,[data-is="app-search"] header > .prev{ left: 15px; } app-search header > .next,[data-is="app-search"] header > .next{ right: 15px; } app-search header > a,[data-is="app-search"] header > a{ text-decoration: none; } app-search header > span.prev,[data-is="app-search"] header > span.prev,app-search header > span.next,[data-is="app-search"] header > span.next{ color: rgba(255, 255, 255, .2); border-color: rgba(255, 255, 255, .1); background: none; } app-search ul,[data-is="app-search"] ul{ list-style: none; padding: 0; text-align: center; } app-search li,[data-is="app-search"] li{ display: inline-block; margin: 15px 5px; height: 220px; width: 165px; overflow: hidden; background-size: contain; background-repeat: no-repeat; background-position: bottom; } app-search li > a,[data-is="app-search"] li > a{ display: block; color: transparent; height: 100%; } app-search button,[data-is="app-search"] button{ display: block; background: rgba(255, 255, 255, .1); border: 1px solid rgba(255, 255, 255, .3); border-radius: 3px; color: rgba(255, 255, 255, .8); text-align: center; padding: 1em; margin: 1em auto; width: 80%; max-width: 500px; }', '', function(opts) {
    ...
    var twoWeekAgo = fecha.format(d0.setDate(d0.getDate() - 14), 'YYYY-MM-DD');
    var twoWeekLater = fecha.format(d0.setDate(d0.getDate() + 27), 'YYYY-MM-DD');

    this.books = [];
    this.visibleBooks = [];
    this.on('route', function (date) {
      date = date || fecha.format(new Date(), 'YYYY-MM-DD');
      var d = fecha.parse(date, 'YYYY-MM-DD');
      var jDate = fecha.format(d, 'YYYY/M/D (ddd)');
      ...
      fetch(("data/" + date + ".json"))
        ...
        .then(function (books) {
          ...
          var visibleBooks = books.slice(0, limit);
          this$1.update({books: books, visibleBooks: visibleBooks});
        });
    });

    this.more = function (e) {
      var n = this$1.visibleBooks.length + limit;
      this$1.visibleBooks = this$1.books.slice(0, n);
    };
});

riot$1.tag2('app-detail', '<header> <h2>本の詳細</h2> <a class="prev" href="#{date}">戻る</a> </header> <img riot-src="{cover}"> <h3>{title}</h3> <p>{description}</p> <ul> <li>{author}</li> <li if="{price}">価格 &yen;{price}</li> <li>ISBN {isbn}</li> </ul>', 'app-detail,[data-is="app-detail"]{ display: block; } app-detail header,[data-is="app-detail"] header{ text-align: center; margin: 0 0 5px; border-bottom: 1px solid rgba(255, 255, 255, .1); color: rgba(255, 255, 255, .8); position: relative; } app-detail header > h2,[data-is="app-detail"] header > h2{ font-size: 100%; padding: 15px 10px; } app-detail header > .prev,[data-is="app-detail"] header > .prev{ position: absolute; background: rgba(255, 255, 255, .1); border: 1px solid rgba(255, 255, 255, .3); border-radius: 3px; color: rgba(255, 255, 255, .8); text-align: center; padding: 5px 10px; top: 10px; } app-detail header > .prev,[data-is="app-detail"] header > .prev{ left: 15px; } app-detail header > a,[data-is="app-detail"] header > a{ text-decoration: none; } app-detail img,[data-is="app-detail"] img{ width: 200px; } app-detail > h3,[data-is="app-detail"] > h3{ margin: 15px; padding: 0 0 5px; border-bottom: 1px solid rgba(255, 255, 255, .3) } app-detail > p,[data-is="app-detail"] > p{ margin: 15px; } app-detail > img,[data-is="app-detail"] > img{ display: block; margin: 15px auto; } app-detail > ul,[data-is="app-detail"] > ul{ color: rgba(255, 255, 255, .7) }', '', function(opts) {
    var this$1 = this;

    this.on('route', function (date, isbn) {
      this$1.update({date: date, isbn: isbn});

      fetch(("data/" + date + ".json"))
        .then(function (response) { return response.json(); })
        .then(function (books) {
          var book = books.find(function (book) { return book.isbn === isbn; });
          this$1.update(book);
        });
    });
});
